...
module "vpc" {
  ...
  name = "terraform-gems-vpc"
  cidr = "10.0.0.0/16"

 #Establishing different AZS and associating them with public/private subnets 
  azs             = ["us-east-1a", "us-east-1b"] 
  private_subnets = ["10.0.3.0/24", "10.0.4.0/24"] #Private subnets for Web Sever Tier
  public_subnets  = ["10.0.1.0/24", "10.0.2.0/24"] #Public subnets 
  private_subnets2 = ["10.0.5.0/24", "10.0.6.0/24"] #Private subnets for the RDS Tier
